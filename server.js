...
app.use((req, res, next) => {
  ...
  if (
    ...
    req.query.key != process.env.APP_SECRET_KEY
  ) {
...
app.get(
  ...
  async (req, res) => {
    ...
    let cached = await client.get(`discord_lookup:guild:${id}`);
      ...
      fetch(`https://canary.discord.com/api/v10/guilds/${id}/widget.json`, {
        ...
        .then((json) => {
          if (json.code && json.code === 50004) {
            res.send({
              error:
                "The guild is either non-existant, unavailable, or has Server Widget/Discovery disabled.",
            });
            return;
          }
...
app.get(
  ...
  async (req, res) => {
    ...
    let cached = await client.get(`discord_lookup:application:${id}`);

    if (cached) res.send(JSON.parse(cached));
    else {
      fetch(
        `https://canary.discord.com/api/v10/applications/${req.params.id}/rpc`,
        {
          headers: {
            "Content-Type": "application/json",
          },
...
app.get(
  ...
  async (req, res) => {
    ...
    try {
      let cached = await client.get(`discord_lookup:user:${id}`);
          ...
          .then((json) => {
            ...
            let output = {
              ...
              avatar: {
                ...
                link: avatarLink,
                is_animated:
                  json.avatar != null && json.avatar.startsWith("a_")
                    ? true
                    : false,
              },
              ...
              banner: {
                ...
                link: bannerLink,
                is_animated:
                  json.banner != null && json.banner.startsWith("a_")
                    ? true
                    : false,
